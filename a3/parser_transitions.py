...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            head = self.stack[stack_len-1] # Last element in stack
            dependent = self.stack[stack_len-2] # Second-to-last element in stack
            self.dependencies.append((head, dependent))
            self.stack.pop(stack_len-2) # Remove dependent
        elif transition == "RA":
            stack_len = len(self.stack)
            head = self.stack[stack_len-2] # Second-to-last element in stack
            dependent = self.stack[stack_len-1] # Last element in stack
            self.dependencies.append((head, dependent))
            self.stack.pop(stack_len-1) # Remove dependent
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        unfinished_parses_len = len(unfinished_parses)
        unfinished_parses_rem = unfinished_parses_len % batch_size
        for mini_batch_t in range(int(unfinished_parses_len / batch_size)):
            mini_batch = unfinished_parses[mini_batch_t*batch_size:(mini_batch_t+1)*batch_size]
            tranisitions = model.predict(mini_batch)
            for i, transition in enumerate(tranisitions):
                unfinished_parses[i+mini_batch_t*batch_size].parse_step(transition)
        if unfinished_parses_rem != 0:
            mini_batch = unfinished_parses[unfinished_parses_len - unfinished_parses_rem: unfinished_parses_len]
            tranisitions = model.predict(mini_batch)
            for i, transition in enumerate(tranisitions):
                unfinished_parses[i + unfinished_parses_len - unfinished_parses_rem].parse_step(transition)
        for i in range(unfinished_parses_len-1, -1, -1):
