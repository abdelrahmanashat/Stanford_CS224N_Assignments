...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        target_vocab_size = len(self.vocab.tgt) # This is the target vocabulary size, called V_{t} in the PDF.
        ...
        self.target_vocab_projection = nn.Linear(in_features=self.hidden_size, out_features=target_vocab_size, bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # 1. Constructing Tensor `X`
        X = self.model_embeddings.source(source_padded)
        
        # 2. Applying the post_embed_cnn layer
        X = torch.permute(X, (1, 2, 0))
        X = self.post_embed_cnn(X)
        X = torch.permute(X, (2, 0, 1))
        
        # 3. Computing `enc_hiddens`, `last_hidden`, `last_cell` by applying the encoder to `X`.
        ...
        # 4. Computing `dec_init_state` = (init_decoder_hidden, init_decoder_cell)
        [last_hidden_forward, last_hidden_backward] = last_hidden
        init_decoder_hidden = self.h_projection(torch.cat((last_hidden_forward, last_hidden_backward),dim=1))
        [last_cell_forward, last_cell_backward] = last_cell
        init_decoder_cell = self.c_projection(torch.cat((last_cell_forward, last_cell_backward),dim=1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # 1. Applying the attention projection layer to `enc_hiddens` to obtain `enc_hiddens_proj`
        enc_hiddens_proj = self.att_projection(enc_hiddens)
        
        # 2. Constructing tensor `Y` of target sentences with shape (tgt_len, b, e) using the target model embeddings
        Y = self.model_embeddings.target(target_padded)
        
        # 3. Using the torch.split function to iterate over the time dimension of Y
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 1. Applying the decoder to `Ybar_t` and `dec_state`to obtain the new dec_state
        dec_state = self.decoder(Ybar_t, dec_state)
        
        # 2. Splitting dec_state into its two parts (dec_hidden, dec_cell)
        dec_hidden, dec_cell = dec_state
        
        # 3. Computing the attention scores e_t, a Tensor shape (b, src_len)
        ...
        # 1. Applying softmax to e_t to yield alpha_t
        alpha_t = F.softmax(e_t, dim=1)
        
        # 2. Using batched matrix multiplication between alpha_t and enc_hiddens to obtain the attention output vector, a_t
        a_t = torch.squeeze(torch.bmm(torch.unsqueeze(alpha_t, dim=1), enc_hiddens), dim=1)
        
        # 3. Concatenating dec_hidden with a_t to compute tensor U_t
        U_t = torch.cat((dec_hidden, a_t), dim=1)
        
        # 4. Applying the combined output projection layer to U_t to compute tensor V_t
        V_t = self.combined_output_projection(U_t)
        
        # 5. Computing tensor O_t by first applying the Tanh function and then the dropout layer
