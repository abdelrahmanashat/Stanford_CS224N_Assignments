...
def naiveSoftmaxLossAndGradient(
    ...
    y_vector = np.zeros(outsideVectors.shape[0],) # Shape is (num words in vocab, )
    y_vector[outsideWordIdx] = 1
    y_hat_vector = softmax(np.dot(outsideVectors, centerWordVec.T)) # Shape is (num words in vocab, )
    loss = -np.log(y_hat_vector[outsideWordIdx])
    gradCenterVec = np.dot(outsideVectors.T, (y_hat_vector - y_vector))
    gradOutsideVecs = np.dot(np.expand_dims((y_hat_vector - y_vector),axis=1), np.expand_dims(centerWordVec,axis=1).T)
...
def negSamplingLossAndGradient(
    ...
    indices = [outsideWordIdx] + negSampleWordIndices  # The first element is outsideWordIdx and the rest are negSampleWordIndices

    ### YOUR CODE HERE (~10 Lines)
    u_o = outsideVectors[outsideWordIdx]
    u_ws = outsideVectors[np.array(negSampleWordIndices)]
    v_c = centerWordVec
    
    loss = - np.log(sigmoid(np.dot(u_o.T, v_c))) - np.sum(np.log(sigmoid(-np.dot(u_ws, v_c))))
    gradCenterVec = -(1 - sigmoid(np.dot(u_o.T, v_c))) * u_o + np.sum((1 - sigmoid(-np.dot(u_ws, v_c))) * u_ws.T, axis=1)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    unique_values, unique_value_indices, counts = np.unique(negSampleWordIndices, return_counts=True, return_index=True)
    gradOutsideVecs[outsideWordIdx] = -(1-sigmoid(np.dot(u_o.T, v_c))) * v_c
    gradOutsideVecs[unique_values] = ((1-sigmoid(-np.dot(u_ws[unique_value_indices], v_c))) * counts)[:, np.newaxis] * v_c[np.newaxis, :] 
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        loss_word, gradCenterVec_word, gradOutsideVecs_word = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]], word2Ind[word], outsideVectors, dataset)
        loss += loss_word
        gradCenterVecs[word2Ind[currentCenterWord]] += gradCenterVec_word
